...
export default function RegisterRestaurant() {
  ...
  useEffect(() => {
    setMounted(true);
    // Detect if this page is loaded inside the Ad Studio iframe
    setIsIframe(window.self !== window.top);

    const fetchEventsAndAds = async () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newRestaurant = {
      ...
      other_options: formData.getAll('other_options'),
  ...
  return (
    <div className="w-full">
      {/* PORTALED AD LAYER: Mounts to document.body to ensure perfect Y=0 coordinates, hides if in Editor */}
            ...
            <section className="p-8 md:p-10 bg-purple-50 border border-purple-100 rounded-[32px] space-y-6">
              ...
              <div className="flex flex-col gap-3">
                {activeEvents.map(event => (
                  <label key={event.id} className="flex items-start cursor-pointer p-4 rounded-2xl border border-purple-200 bg-white hover:bg-purple-50 hover:border-purple-300 transition-all duration-200">
                    <div className="flex items-center h-6 mt-1">
                      <input type="checkbox" name="other_options" value={event.name} className="w-5 h-5 accent-purple-600 rounded" />
                    </div>
