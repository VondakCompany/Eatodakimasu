...
export default function Home() {
  ...
  // ✅ SAFELY CALCULATE DISTANCE IN JS
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      ...
      // ALWAYS USE STANDARD DB QUERY (No RPC)
      ...
      if (userLocation) {
        // ✅ GEOLOCATION OVERRIDE: Fetch all matching (up to 1000), calculate math, and sort locally.
        const { data, error } = await dbQuery.limit(1000);
        if (!error && data) {
          let processed = data
            .filter(r => r.lat && r.lng) // Must have coordinates
            .map(r => ({
              ...r,
          ...
          setHasMore(false); // Display all sorted at once
        } else if (error) {
  ...
  const lastElementRef = useCallback((node: HTMLDivElement | null) => {
    if (loading || userLocation) return; // Disable observer if geolocating (already loaded all)
                ...
                <div className="flex flex-wrap gap-2">
                  {group.options.map((opt) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition ${group.state.includes(opt.name) ? 'bg-orange-600 text-white border-orange-600 shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'}`}
                    >
            ...
            {restaurants.length === 0 && !loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {restaurants.map((restaurant, index) => {
                  ...
                  return <RestaurantCard key={restaurant.id} restaurant={restaurant} activeEvents={activeEvents} />;
                })}
              </div>
            )}
