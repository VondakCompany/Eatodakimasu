...
export default function RestaurantPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleShare = async () => {
    const shareData = {
      title: displayTitle,
      text: displayDescription?.substring(0, 50) + '...',
      ...
      {lightboxOpen && restaurant.image_urls && (
        ...
        >
          ...
          {restaurant.image_urls.length > 1 && (
            <button 
              className="absolute left-4 md:left-8 text-white/50 hover:text-white text-5xl md:text-6xl p-4 transition-colors z-50 select-none"
              onClick={prevImage}
            >
              ‹
            </button>
          ...
          {restaurant.image_urls.length > 1 && (
            <button 
              className="absolute right-4 md:right-8 text-white/50 hover:text-white text-5xl md:text-6xl p-4 transition-colors z-50 select-none"
              onClick={nextImage}
            >
              ›
            </button>
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden mt-4 md:mt-8 mb-20">
        ...
        <div className="h-64 md:h-96 w-full relative bg-gray-900 group">
          ...
          <div className="absolute top-6 left-6 right-6 flex justify-between z-10">
            ...
            <button 
              onClick={handleShare}
              className="bg-white/90 backdrop-blur text-gray-900 px-4 py-2 rounded-xl font-bold text-sm hover:bg-white transition shadow-sm flex items-center gap-2"
            >
              <span>↗</span> {t('btn_share', 'シェア')}
            </button>
          </div>
        </div>
        ...
        <div className="p-8 md:p-12">
          ...
          {/* DEDICATED GALLERY THUMBNAILS SECTION */}
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                {restaurant.image_urls.map((url: string, idx: number) => (
                  <button 
                    ...
                    className="aspect-square rounded-2xl overflow-hidden border border-gray-200 shadow-sm bg-gray-50 group focus:outline-none focus:ring-4 focus:ring-orange-500/30"
                  >
                    <img 
                      src={url} 
                      alt={`${displayTitle} thumbnail ${idx + 1}`} 
                      className="w-full h-full object-cover group-hover:scale-110 group-hover:opacity-90 transition duration-300" 
                    />
                  </button>
                ))}
              </div>
